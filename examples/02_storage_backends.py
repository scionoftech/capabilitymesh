...
async def demo_in_memory_storage():
    ...
    await mesh.register(calculate, name="calculator", capabilities=["math", "calculation"])
    print("[OK] Registered calculator agent")

    # Discover and execute
    agents = await mesh.discover("math")
    print(f"[OK] Found {len(agents)} math agents")

    result = await mesh.execute(agents[0].id, "2 + 2 * 3")
    print(f"[OK] Calculation result: 2 + 2 * 3 = {result}")
...
async def demo_sqlite_storage():
    ...
    await mesh.register(process_text, name="text-processor", capabilities=["text", "processing"])
    await mesh.register(count_words, name="word-counter", capabilities=["text", "analysis"])
    print("[OK] Registered 2 text agents")

    # Full-text search
    agents = await mesh.discover("text processing")
    print(f"[OK] Full-text search found {len(agents)} agents")
    for agent in agents:
        print(f"  - {agent.name}")
    ...
    if agents:
        ...
        print(f"[OK] Processing result: {result}")
    ...
    agents2 = await mesh2.list_agents()
    print(f"  [OK] Loaded {len(agents2)} agents from disk")
...
async def demo_redis_storage():
    ...
    try:
        ...
        await mesh.register(call_api, name="api-caller", capabilities=["api", "http"])
        await mesh.register(manage_cache, name="cache-manager", capabilities=["cache", "storage"])
        print("[OK] Registered 2 agents to Redis")

        # Discover
        agents = await mesh.discover("api")
        print(f"[OK] Found {len(agents)} API agents")
